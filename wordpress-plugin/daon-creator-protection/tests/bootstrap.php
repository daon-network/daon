...
// Standalone mode needs somewhere for ABSPATH to point, and the plugin's
// activation path does `require_once ABSPATH . 'wp-admin/includes/upgrade.php'`
// before calling dbDelta(). Pointing at a directory that does not exist made
// every activation test fail with "Failed to open stream", so the scaffold is
// created here rather than assumed. sys_get_temp_dir() rather than a hardcoded
// /tmp so the suite runs on any platform.
if (!defined('ABSPATH')) {
    $daon_fake_wp = sys_get_temp_dir() . '/daon-wp-test-scaffold/';
    $daon_upgrade_dir = $daon_fake_wp . 'wp-admin/includes/';

    if (!is_dir($daon_upgrade_dir)) {
        mkdir($daon_upgrade_dir, 0777, true);
    }

    $daon_upgrade_file = $daon_upgrade_dir . 'upgrade.php';
    if (!file_exists($daon_upgrade_file)) {
        file_put_contents($daon_upgrade_file, <<<'STUB'
<?php
// Generated by tests/bootstrap.php for standalone runs. Not WordPress.
// dbDelta() is a no-op here: these tests assert the plugin's own behaviour,
// not that WordPress can apply a schema.
